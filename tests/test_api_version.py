...
def test_use_new_api_true_for_new_engine(monkeypatch):
    ...
    assert use_new_api(NEW_API_MIN_VERSION) is True


def test_use_new_api_false_for_old_engine(monkeypatch):
    monkeypatch.delenv("CFX_API_VERSION_1", raising=False)
    assert use_new_api(CFXVersion.v252) is False


def test_use_new_api_env_override(monkeypatch):
    monkeypatch.setenv("CFX_API_VERSION_1", "1")
    assert use_new_api(CFXVersion.v252) is True
...
def test_detect_engine_version_v1_success(monkeypatch):
    ...
    monkeypatch.setattr(api_version, "_probe_one", fake_probe)
...
def test_detect_engine_version_raises_when_all_fail(monkeypatch):
    monkeypatch.delenv("CFX_API_VERSION_1", raising=False)
    monkeypatch.setattr(api_version, "_probe_one", lambda *a, **k: None)
    with pytest.raises(RuntimeError, match="Could not detect CFX engine version"):
        detect_engine_version(mock.Mock(), [])


def test_probe_one_returns_none_on_fallback_status(monkeypatch):
    stub = mock.Mock()
    ...
    assert api_version._probe_one(mock.Mock(), [], "v1") is None


def test_probe_one_reraises_unexpected_rpc_error(monkeypatch):
    stub = mock.Mock()
    ...
    with pytest.raises(grpc.RpcError):
        api_version._probe_one(mock.Mock(), [], "v1")
